...
async function login(apiRequest: request.APIRequestContext) {
...
test.describe('Admin Pending Adjustment E2E (API)', () => {
  test('create → list → approve', async ({ request: api }) => {
    ...
    // create test pending
    const create = await api.post(`${API}/api/adminpanel/pending-adjustments/test-create`, {
    ...
    // approve
    const approve = await api.post(`${API}/api/adminpanel/pending-adjustments/${pendingId}/approve?approvedBy=e2e`, {
      headers: { Authorization: `Bearer ${token}` }
    });
  ...
  test('create → reject', async ({ request: api }) => {
    ...
    // reject
    const reject = await api.post(`${API}/api/adminpanel/pending-adjustments/${pendingId}/reject`, {
