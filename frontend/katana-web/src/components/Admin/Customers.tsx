...
const Customers: React.FC = () => {
  ...
  const [lucaInfoDialog, setLucaInfoDialog] = useState<{
    open: boolean;
  ...
  const fetchCustomers = async () => {
    try {
      ...
      setError(
        error.response?.data?.message || "Müşteriler yüklenirken hata oluştu"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSyncToLuca = async (customer: Customer) => {
    ...
    try {
      const response = await api.post<SyncResult>(
        `/customers/${customer.id}/sync`
      );
      if (response.data.isSuccess) {
        setSnackbar({
          open: true,
          message: "Luca senkronizasyonu başarılı",
          severity: "success",
        });
      } else {
        setSnackbar({
          open: true,
          message: response.data.message || "Senkronizasyon hatası",
          severity: "error",
        });
      }
      fetchCustomers();
  ...
  const getLucaStatusChip = (customer: Customer) => {
    const status =
      customer.lucaSyncStatus ||
      (customer.lucaFinansalNesneId && !customer.lastSyncError
        ? "success"
  ...
  return (
    <Box>
          ...
          <Box sx={{ ...gridStyles.container, mt: 2 }}>
            ...
            <FormControl fullWidth>
              ...
              <Select
                ...
                label="Müşteri Tipi"
                onChange={(e) =>
                  setFormData({ ...formData, type: e.target.value as number })
                }
              >
                <MenuItem value={1}>
      ...
      >
          ...
          {lucaInfoDialog.customer && (
            <Box sx={{ ...gridStyles.container, mt: 2 }}>
              ...
              <Box>
                ...
                <Typography variant="body1" fontWeight="bold">
                  {lucaInfoDialog.customer.lucaFinansalNesneId ||
                    "Henüz atanmadı"}
                </Typography>
              </Box>
              <Box sx={gridStyles.fullWidth}>
        ...
        <DialogActions>
          <Button
            onClick={() => setLucaInfoDialog({ open: false, customer: null })}
          >
            Kapat
          </Button>
          {lucaInfoDialog.customer && (
